...
# Стабильно работает этот метод только в браузерах FireFox и Opera.
    ...
    def login(self):
        ...
        if not cookies:
            try:
                ...
                if str(cookies).find("is_auth=true") == -1:
                    print(
                        "Откройте браузер FireFox или Opera, авторизируйтесь в ЭЖД,\nперейдите в справочник \"Кадры\" и запустите программу снова!")
                    self.free()
                    return False
            except browser_cookie3.BrowserCookieError:
