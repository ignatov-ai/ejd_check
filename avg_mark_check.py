...
for file in file_list:
    ...
    avg_book_sheet.column_dimensions['A'].width = 35
    ...
    # считаем количество ошибок в классе
    errors = 0
    ...
    for sheet_name in tabs:
        ...
        row_max = 3
        while sheets['A'+str(row_max)].value != None:
            row_max += 1
        row_max -= 1

        #print(row_max, lesson)

        # считаем количество непустых столбцов в строке
        col_max = 100
        while True:
            #print(get_column_letter(col_max) + '4', sheets[get_column_letter(col_max) + '4'].value)
            col_max -= 1
            if sheets[get_column_letter(col_max) + '4'].value not in [None, '']:
                break

        #print('Последний не пустой столбец', get_column_letter(col_max), lesson, 'На этом листе количество столбцов =', col_max)

        # пропуск листа, если стоит только АП
        if col_max < 10:
            continue
        ...
        for row in range(3, row_max+1):
            ...
            try:
                ...
                if avg_class_data[avg_row_student_id][avg_row_lesson_id+1] == student_mark_in_ap:
                    ...
                    avg_book_sheet[get_column_letter(avg_row_lesson_id + 2) + str(avg_row_student_id + 7)].value = str(avg_class_data[avg_row_student_id][avg_row_lesson_id+1]) + ' (' + str(student_mark_in_ap) + ')'
